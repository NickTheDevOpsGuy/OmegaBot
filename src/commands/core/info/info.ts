...
async function handleTime(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const db = getDb();
  const tz = getTimezone(db, user.id);

  if (!tz) {
    const hint =
      user.id === interaction.user.id
        ? "Set your timezone with `/profile timezone zone:<iana>` (e.g. America/New_York)."
        : "They haven't set a timezone. Ask them to use `/profile timezone`.";
    await interaction.editReply({
      content: `${user} doesn't have a timezone set.\n${hint}`,
    });
    return;
  }
  ...
  try {
    ...
    await interaction.editReply({
      content: `Invalid timezone stored for ${user}. They can update it with \`/profile timezone\`.`,
    });
    return;
  }
