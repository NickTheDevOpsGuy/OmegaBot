 ...
 *
 * Goal: keep output readable and under Discord limits by splitting into topics.
 */
...
function buildOverviewHelp(args: { isAdmin: boolean }): string {
  ...
  lines.push("overview, fun, github, summary, timezone" + (isAdmin ? ", admin" : ""));
  lines.push("");
...
function buildFunHelp(): string {
  ...
  lines.push("**Help: Fun**");
  lines.push("");
  lines.push("All fun commands are under `/fun`.");
  lines.push("");
...
function buildCommandsHelp(args: {
  ...
  lines.push("**Help: Registered commands**");
  lines.push("");
  lines.push("This is a sanity list of top-level commands currently loaded.");
  lines.push("");
