...
export async function handleJokeRemove(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const hasModRole = modRoleId && 'cache' in member.roles && member.roles.cache.has(modRoleId);
