...
type TriviaRow = { correct: number; incorrect: number; points: number; best_streak: number };
type DailyRow = { streak: number; best_streak: number; total_checkins: number; points: number };
...
type WordleRow = { played: number; won: number; current_streak: number; max_streak: number };
...
function getRPSStats(db: ReturnType<typeof getDb>, userId: string): RPSRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT wins, losses, ties FROM rps_stats WHERE user_id = ?`).get(userId) as RPSRow,
  );
}

function getTriviaStats(db: ReturnType<typeof getDb>, userId: string): TriviaRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT correct, incorrect, points, best_streak FROM trivia_stats WHERE user_id = ?`).get(userId) as TriviaRow,
  );
}

function getDailyStats(db: ReturnType<typeof getDb>, userId: string): DailyRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT streak, best_streak, total_checkins, points FROM daily_checkins WHERE user_id = ?`).get(userId) as DailyRow,
  );
}

function getTTTStats(db: ReturnType<typeof getDb>, userId: string): TTTRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT wins, losses, ties FROM ttt_stats WHERE user_id = ?`).get(userId) as TTTRow,
  );
}

function getBlackjackStats(db: ReturnType<typeof getDb>, userId: string): BlackjackRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT wins, losses, ties, blackjacks FROM blackjack_stats WHERE user_id = ?`).get(userId) as BlackjackRow,
  );
}

function getHangmanStats(db: ReturnType<typeof getDb>, userId: string): HangmanRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT wins, losses, total_guesses FROM hangman_stats WHERE user_id = ?`).get(userId) as HangmanRow,
  );
}

function getWordleStats(db: ReturnType<typeof getDb>, userId: string): WordleRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT played, won, current_streak, max_streak FROM wordle_stats WHERE user_id = ?`).get(userId) as WordleRow,
  );
}

function getSlotsStats(db: ReturnType<typeof getDb>, userId: string): SlotsRow | null {
  return safeQuery(() =>
    db.prepare(`SELECT spins, wins, jackpots FROM slots_stats WHERE user_id = ?`).get(userId) as SlotsRow,
  );
}
...
function getCoinStats(db: ReturnType<typeof getDb>, userId: string): CoinRow | null {
  return safeQuery(() => {
    const heads = db
      .prepare(`SELECT COUNT(*) as count FROM coin_flips WHERE user_id = ? AND result = 'heads'`)
      .get(userId) as { count: number };
    const tails = db
      .prepare(`SELECT COUNT(*) as count FROM coin_flips WHERE user_id = ? AND result = 'tails'`)
      .get(userId) as { count: number };
...
export async function run(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (ttt) {
    ...
    gameLines.push(`⭕ **Tic-Tac-Toe:** ${ttt.wins}W/${ttt.losses}L/${ttt.ties}T (${winRate}%)`);
  }

  if (blackjack) {
    const total = blackjack.wins + blackjack.losses + blackjack.ties;
    const winRate = total > 0 ? Math.round((blackjack.wins / total) * 100) : 0;
    gameLines.push(`🃏 **Blackjack:** ${blackjack.wins}W/${blackjack.losses}L (${winRate}%) | 🎰 ${blackjack.blackjacks}`);
  }
  ...
  if (wordle) {
    const winRate = wordle.played > 0 ? Math.round((wordle.won / wordle.played) * 100) : 0;
    gameLines.push(`🟩 **Wordle:** ${wordle.won}/${wordle.played} (${winRate}%) | 🔥 ${wordle.current_streak}`);
  }
  ...
  if (slots) {
    ...
    funLines.push(`🎰 **Slots:** ${slots.spins} spins, ${slots.wins} wins (${winRate}%) | 💎 ${slots.jackpots} jackpots`);
  }
  ...
  // No stats at all
  if (!rps && !trivia && !daily && !ttt && !blackjack && !hangman && !wordle && !slots && !coins) {
