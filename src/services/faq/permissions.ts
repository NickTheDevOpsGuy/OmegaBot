...
export function canFaqAction(
  ...
  if (!interaction.inGuild()) {
    return { ok: false, reason: "`/faq add/remove` can only be used in a server." };
  }

  const perms = interaction.memberPermissions;
  if (!perms) {
    return { ok: false, reason: "Could not resolve your permissions for this server." };
  }
  ...
  if (!ok) {
    return { ok: false, reason: "You need Manage Server or Administrator to do that." };
  }
